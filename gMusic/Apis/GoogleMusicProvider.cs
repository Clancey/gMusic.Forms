	...
	internal class GoogleMusicProvider : MusicProvider
	{
		...
		public GoogleMusicProvider(GoogleMusicApi api) : base(api)
		{
			Connectivity.ConnectivityChanged += async (sender, args) =>
			{
				...
				if (args.NetworkAccess == NetworkAccess.None) return;
		...
		protected override async Task<bool> Sync()
		{
			try
			{
				...
				//TODO: bring back if using Youtube
				//Decipherer.PreCache();
		...
		async Task<bool> GetTracks(string token = null)
		{
			try
			{
				...
				var resp = await SyncRequestQueue.Enqueue(() => Api.GetLatest<RootTrackApiObject>(path, query, includeDeviceHeaders: true));
		...
		async Task<bool> GetThumbsUpTracks()
		{
			try
			{
				...
				var resp = await SyncRequestQueue.Enqueue(() => Api.PostLatest<RootTrackApiObject>(path, request, query));
				var youtubeTracks = new List<FullTrackData>();
				var start = DateTime.Now;
				var tracks = resp?.Data?.Items?.Select(x =>
				{
		...
		async Task<bool> GetPlaylists(string token = null)
		{
			try
			{
				...
				var resp = await SyncRequestQueue.Enqueue(() => Api.GetLatest<RootPlaylistApiObject>(path, query));
				Task<bool> nextTask = null;
				if (!string.IsNullOrWhiteSpace(resp?.NextPageToken))
		...
		async Task<bool> GetPlaylistTracks(string token = null)
		{
			try
			{
				...
				//TODO: Bring back when we get an API Manager
				//ApiManager.Shared.SaveApi(Api);
				return success;
			}
			catch (Exception ex)
		...
		public Task<bool> SyncSharedPlaylistTracks()
		{
			return Task.Run(async () =>
			{
				try
				{
					...
					var resp = await SyncRequestQueue.Enqueue(() => Api.PostLatest<RootOnlinePlaylistApiObject>(path, request, query));
		...
		async Task<bool> GetRadioStations(string token = null)
		{
			try
			{
				...
				var resp = await SyncRequestQueue.Enqueue(() => Api.PostLatest<RootRadioStationsApiObject>(path, request, query));
				if (resp?.Error != null)
				{
					Console.WriteLine(resp.Error);
					return false;
		...
		public override async Task<bool> LoadRadioStation(RadioStation station, bool isContinuation)
		{
			...
			//	var resp = await SyncRequestQueue.Enqueue(() => Api.PostLatest<RootRadioStationsTracksApiObject>(path, request, query));
			//	if (resp?.Error != null)
			//	{
			//		Console.WriteLine(resp.Error);
			//		return false;
		...
		protected async Task<bool> ProcessStationTracks(RadioStation station, List<SongItem> tracks)
		{
			...
			var order =
				Database.Main.ExecuteScalar<int>("select count(*) from RadioStationSong where StationId = ?",
					station.Id);
			tracks.ForEach(s =>
			{
			...
			try
			{
				...
				var data = new DownloadUrlData
				{
					...
					Headers =
					{
						...
						{"X-Device-FriendlyName", Utility.DeviceName},
		...
		public override async Task<Uri> GetPlaybackUri(Track track)
		{
			...
			if (!Api.HasAuthenticated)
				await Api.Authenticate();
			var wifi =  Connectivity.ConnectionProfiles.Any(x=> x == ConnectionProfile.WiFi);
