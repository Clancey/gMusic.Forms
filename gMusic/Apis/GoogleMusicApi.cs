...
namespace gMusic.Api.GoogleMusic
{
	...
	internal class GoogleMusicApi : GoogleApi
	{
		...
		public string DeviceName {
			get {
				if(string.IsNullOrWhiteSpace(deviceName))
					deviceName = Utility.GetSecured ("deviceName");
			...
			set {
				deviceName = value;
				Utility.SetSecured ("deviceName", value);
			}
		}
			...
			: base(id, "936475272427", "KWsJlkaMn1jGLxQpWxMnOox-", handler)
		{
			...
			{
				if (Connectivity.NetworkAccess != NetworkAccess.None && this.CurrentAccount == null)
				{
					Authenticate();
				}
			};
...
//				Logger.Log(ex);
		...
		public async Task<string> GetDeviceId(bool next = false)
		{
			try
			{
				...
				{
                    var json =  await Get("https://mclients.googleapis.com/sj/v1.11/devicemanagementinfo");
                    var settings = await Get<SettingsRootObject>("https://mclients.googleapis.com/sj/v1.11/devicemanagementinfo");
					if (settings != null)
					{
