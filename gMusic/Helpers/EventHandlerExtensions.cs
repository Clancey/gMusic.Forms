...
namespace gMusic
{
	internal static class EventHandlerExtensions
	{
		...
		public static void InvokeOnMainThread<T>(this EventHandler<EventArgs<T>> handler, object sender, T args, [CallerMemberName] string memberName = "",
							   [CallerFilePath] string sourceFilePath = "",
							   [CallerLineNumber] int sourceLineNumber = 0)
		{
			Device.BeginInvokeOnMainThread (() =>
			{
				try{
					using (new EventLogger(memberName))
						handler?.Invoke(sender, new EventArgs<T>(args));
				}
				catch (Exception ex)
				{
					LogManager.Shared.Report(ex);
