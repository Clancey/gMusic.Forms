...
namespace gMusic.Managers {
	public class PlaybackManager : ManagerBase<PlaybackManager> {
		...
		public void Init ()
		{
			...
			//TODO: Bring the native handlers back
			//#if __MACOS__
		...
		public Timer ProgressTimer {
			get {
				...
				progressTimer.Elapsed += (sender, args) => {
					...
					if (Player.State == PlaybackState.Playing && p > 0)
		...
		public Timer VisualizerTimer {
			get {
				...
				visualizerTimer.Elapsed +=
					(sender, args) => NotificationManager.Shared.ProcUpdateVisualizer (Player?.AudioLevels ?? new float [] { 0, 0 });
		...
		public async Task PlayNow (Song song, bool playVideo = false)
		{
			...
			if (song.Id == Settings.CurrentSong) {
				await Player.PlaySong (song, playVideo);
				return;
			}
		...
		{
			if (groupInfo == null)
				groupInfo = Database.Main.CreateGroupInfo (playlist, Settings.ShowOfflineOnly);
			LogManager.Shared.LogPlayback (playlist);
			SendEndNotification (PlaybackEndedReason.Skipped);
			Settings.CurrentPlaybackContext = new PlaybackContext {
				IsContinuous = false,
		...
		public async Task PrepareNextTrack ()
		{
			try {
				...
				Player.Queue (playbackData.CurrentTrack);
		...
		public async Task NextTrackWithoutPause (bool notification = false, bool playVideo = false)
		{
			...
			if (notification) {
				SendEndNotification (nextIndex == CurrentSongIndex
					? PlaybackEndedReason.StartedOver
					: PlaybackEndedReason.Skipped);
			}
			if (nextIndex == -1) {
				Player.Seek (0);
				Pause ();
		...
		public void Previous ()
		{
			...
			if (diff < 5 || Player.CurrentTimeSeconds () < 5)
				PlayPrevious ();
			else {
				SendEndNotification (PlaybackEndedReason.StartedOver);
				Seek (0);
		...
		{
			SendEndNotification (PlaybackEndedReason.Reverse);
		...
		public async Task LoadMoreTracks ()
		{
			...
			loadMoreTracksTask = Task.Run (async () => {
				try {
					...
					string query = $"select SongId as Id from RadioStationSong where StationId = ? order by SOrder";
					await SetupCurrentPlaylist (query, Player?.CurrentSong?.Id ?? "", new object [] { station.Id });
				} catch (Exception ex) {
					LogManager.Shared.Report (ex);
				}
			});
				...
				Task.Run (async () => {
					while (true) {
						if (Player.Duration () > 0) {
							Player.Seek (Settings.CurrentPlaybackPercent);
							break;
						} else
