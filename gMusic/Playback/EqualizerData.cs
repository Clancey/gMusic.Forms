...
{
    public static class EqualizerData
    {
        static EqualizerData()
        {

            LoadPresets();
            CurrentPreset = Settings.EqualizerPreset > 0
                ? Presets.FirstOrDefault(x => x.Id == Settings.EqualizerPreset)
        ...
        public static bool Active
        {
            ...
            set
            {
                if (active == value)
                    return;
                active = value;
                Settings.EqualizerEnabled = active;
                if (active)
                    ApplyEqualizer();
                else
                    Clear();
        ...
        static EqualizerPreset currentPreset;

        public static EqualizerPreset CurrentPreset
        {
        ...
        public static List<EqualizerPreset> Presets = new List<EqualizerPreset>();
        ...
        public static async Task ApplyEqualizer()
        {
        ...
        public static void UpdateBand(int band, float gain)
        {
            Bands[band].Gain = gain;
            if (!Settings.EqualizerEnabled)
                return;
            PlaybackManager.Shared.Player.UpdateBand(band, gain);
        ...
        public static void Clear()
        {
            ...
            {
                PlaybackManager.Shared.Player.UpdateBand(i, 0);
            }
        }
        ...
        public static Band[] ThreeBandPreset()
        {
            return new[]
            {
                new Band {Center = 100},
