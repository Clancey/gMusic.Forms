...
namespace gMusic.iOS.Playback
{
	internal class AVPlayerEqualizer : ManagerBase<AVPlayerEqualizer>
	{
		#region IEqualizer implementation

		protected Band[] Bands = new Band[0];

		AudioTapProcessor processor;



        public async System.Threading.Tasks.Task ApplyEqualizer(Band[] bands, AVPlayerItem item)
		{
			Bands = bands ?? new Band[0];
			if (item == null)
				return;
			...
			processor.IsBandpassFilterEnabled = Settings.EqualizerEnabled;
		...
		public class AudioTapProcessor : NSObject
		{
			...
			{
				PlaybackManager.Shared.Player.AudioLevels = new float[] {leftVolume, rightVolume};
