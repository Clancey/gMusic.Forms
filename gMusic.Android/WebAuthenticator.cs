	...
	public class WebAuthenticatorActivity : Activity {
		...
		class Client : WebViewClient {
			...
			System.Net.Cookie [] GetCookies (string url)
			{
				...
				var cookieString = CookieManager.Instance.GetCookie (url);
				var cookiePairs = cookieString?.Split (';') ?? new string [0];

				var cookies = cookiePairs.Select (x => {
					var parts = x.Split ('=');
					if (parts [0].Contains (":"))
				...
				cookies = cookies.Where (x => x != null).ToArray () ?? new Cookie [0];
				return cookies;
			}
